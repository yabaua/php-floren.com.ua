...
require('../database.php');
    ...
    $data_from_1c = array();
    $dom = new DomDocument;
        $dom->loadHTMLFile($_SERVER['DOCUMENT_ROOT']."/images/1c/_ZakupkiHTML%20(HTML4).html");
...
//exit();
    ...
    foreach ($one_node as $i => $node) {
        ...
        foreach ($node->childNodes as $i => $chnode) {
            ...
            if ($i == 6+$correction-$corretion_filetype) { // was 12
                $price=(float) str_replace(",",".",preg_replace('/[^0-9,]/', '', $chnode->nodeValue));
    ...
    if (count($data_from_1c) > 0) {
        $cnt_goods=0;
        $query="DELETE FROM report_goods_zakupki WHERE report_year='".$report_year."' AND report_month='".$report_month."'";
        //echo "<br><br>";
    	      $db->query($query);
        foreach($data_from_1c AS $k=>$v){
            if (strlen($v['barcode']) != 13) continue;
            else {
          $query="INSERT INTO report_goods_zakupki (barcode,uniqueBarYearMonth,report_year,report_month,qt,gross,price) VALUES
                                       ('".$v['barcode']."','".$v['uniqueBarYearMonth']."','".$v['report_year']."','".$v['report_month']."', '".$v['qt']."', '".$v['gross']."', '".$v['price']."')";
          //	echo $query."<br />";
              $db->query($query, 1);
                $cnt_goods++;
            }
        }
        echo   $sync_text = 'Завантажено куплених товарів: <b>'.$cnt_goods.'</b><br />';
    } else {
        echo '<p class="err">Файл з закупівлями не не знайдено. Дані не оновлено.</p>';
    }
