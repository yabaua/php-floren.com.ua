...
<sl-drawer id="category-drawer" label="Всі категорії" placement="end" class="drawer category-drawer">
  <div class="drawer__content">
    ...
    <section>
      <h3{if $URL[0]=='publications' } class="active" {/if}>
        <a href="{$LANGURL}/publications/">Все</a>
        </h3>
        <ul>
          {foreach item=PC name=PC from=$PUB_CATEGORIES}
          <li {if $PC.act=='1' }class="active" {/if}>
            <a href="{$LANGURL}/publications/?cat={$PC.alias}" class="underline">{$PC.name}</a>
          </li>
          {/foreach}
        </ul>
    </section>
            ...
            <li{if isset($URL[1]) && $URL[1]=='peregorodki-iz-rasteniy' } class="active" {/if}>
              ...
              <li{if isset($URL[1]) && $URL[1]=='ozelenenie-iskusstvennimi-rasteniyami' } class="active" {/if}>
                ...
                <li{if isset($URL[1]) && $URL[1]=='ozelenenie_letney_ploschadki' } class="active" {/if}>
                  <a class="underline"
                    href="{$LANGURL}/services/ozelenenie_letney_ploschadki/">{$LINGVO.ozelenenie_terras}</a>
                  </li>
        </ul>
    </section>
    <section>
      <h3{if isset($URL[1]) && $URL[1]=='vertikalnoe-ozelenenie' } class="active" {/if}>
        ...
        <ul>
          <li{if isset($URL[1]) && $URL[1]=='green-wall' } class="active" {/if}>
            ...
            <li{if isset($URL[1]) && $URL[1]=='vertikalnoe-ozelenenie-metallicheskimi-konstruktsiyami' } class="active"
              {/if}>
              <a class="underline"
                href="{$LANGURL}/services/vertikalnoe-ozelenenie-metallicheskimi-konstruktsiyami/">{$LINGVO.metall_ozel}</a>
              </li>
              <li{if isset($URL[1]) && $URL[1]=='ozelenenie-stabilizirovannim-mhom' } class="active" {/if}>
                <a class="underline"
                  href="{$LANGURL}/services/ozelenenie-stabilizirovannim-mhom/">{$LINGVO.ozelenenie_moss}</a>
                </li>
        </ul>
    </section>
    <section>
