...
function add_body($title,$alias,$gfID,$price,$image,$item_group_id,$hgt=0,$dia=0,$wdt=0) {
	...
	$body.=$tt."<g:description>Купити ".$title.$ttl_dia.$ttl_wdt.$ttl_hgt." в Києві в Студії фітодизайну «Флорен» ".$title." з доставкою по Україні</g:description>".$nn;
	$body.=$tt."<g:link>https://floren.com.ua/ua/".$alias."</g:link>".$nn;
...
$body.='<channel>'.$nn;
$body.=$tt.'<title>Флорен - студія фітодизайну</title>'.$nn;
$body.=$tt.'<link>https://floren.com.ua</link>'.$nn;
$body.=$tt.'<description>Фітодизайн інтереру, продаж кімнатних рослин, горщиків та кашпо.</description>'.$nn;
...
FROM goods_ua g
JOIN goods_ua_class gc ON g.classID=gc.ID
...
WHERE gf.price>0 AND g.availability=1 AND gf.visibility=1
