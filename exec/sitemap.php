...
function add_body($alias,$time,$freq,$priority) {
	...
	$body.="<url>".$nn;
	$body.="<loc>https://floren.com.ua/".$alias."</loc>".$nn;
...
/*
$classID=''; 
$db->query("SELECT * FROM goods_filters_meta WHERE is_index='1' ORDER BY classID");
while($f=$db->fetch()) {
	if($classID!=$f['classID']){
		$db->query("SELECT * FROM goods_class WHERE classID='".$f['classID']."'", 2);
		$ff=$db->fetch(2);
		$sm_url=$ff['alias'];
	}
