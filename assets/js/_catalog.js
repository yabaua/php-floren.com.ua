...
function createProductCard(product) {
  ...
  if (product.colors && product.colors.length > 0) {
    const colorSpans = product.colors
      .map((color) => `<span style="background-color: ${color};" title="${color}"></span>`)
      .join('');
    colorsHtml = `
                    <section>
                        <h5>Кольори:</h5>
  ...
  const optionsBlock =
    variantsHtml || colorsHtml ? `<div class="product-card__options">${variantsHtml}${colorsHtml}</div>` : '';
  return `
...
export { initCatalog as i, updateGoodsList as u };
